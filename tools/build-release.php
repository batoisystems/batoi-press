...
function shouldExclude(string $root, string $path): bool
{
    ...
    if ($relative === 'radpress/tests' || str_starts_with($relative, 'radpress/tests/')) {
        return true;
    }
    if ($basename === '.DS_Store' || $basename === 'installed.lock') {
        return true;
    }
    if (str_starts_with($relative, 'radpress/content/menus/') && str_ends_with($relative, '.json.lock')) {
        return true;
    }

    if (in_array($relative, [
        'radpress/docs/release-management.md',
        'radpress/docs/stable-readiness.md',
    ], true)) {
        return true;
    }
