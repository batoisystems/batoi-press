...
$channel = str_contains($version, '-') ? 'candidate' : 'stable';
$indexName = $channel === 'candidate' ? 'candidate.json' : 'latest.json';
$outputPath = optionValue($argv, '--output') ?? $root . '/dist/' . $indexName;
...
if (is_file($signingKey)) {
    $secret = base64_decode(trim((string)file_get_contents($signingKey)), true);
    if (!is_string($secret)) {
        fwrite(STDERR, "Unable to decode release signing key.\n");
        exit(1);
    }
    $manifest = \Batoi\Press\Update\ReleaseSignature::sign($manifest, $secret, 'batoi-press-release-2026-01', 'release-index');
    $manifest['trust']['signature_url'] = 'https://www.batoi.com/pub/press/' . basename($outputPath) . '.sig';
    $manifest['trust']['public_key_url'] = 'https://www.batoi.com/pub/press/release-public-keys.json';
    $signaturePath = $outputPath . '.sig';
    file_put_contents($signaturePath, (string)$manifest['trust']['signature'] . PHP_EOL, LOCK_EX);
} else {
    fwrite(STDERR, "Release signing key is unavailable: {$signingKey}\n");
    exit(1);
}
...
$publicKeyDocument = json_encode(['algorithm' => 'Ed25519', 'keys' => $publicKeys], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
if (!is_string($publicKeyDocument) || file_put_contents(dirname($outputPath) . '/release-public-keys.json', $publicKeyDocument . PHP_EOL, LOCK_EX) === false) {
    fwrite(STDERR, "Unable to write public release keys.\n");
    exit(1);
}
