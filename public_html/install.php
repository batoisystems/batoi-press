...
if (is_file($lock)) {
    ...
    echo '<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Installer Disabled</title><link rel="stylesheet" href="/assets/css/style.css"></head><body><main class="bp-admin"><h1>Installer Disabled</h1><p>Batoi Press is already installed. Remove <code>config/installed.lock</code> manually on the server to re-enable setup.</p><p><a href="/">View site</a></p></main></body></html>';
    exit;
}
...
$checks = [
    ...
    'Content writable' => is_writable($root . '/content'),
    'Storage writable' => is_writable($root . '/storage'),
    'Themes readable' => is_readable($root . '/themes'),
];
